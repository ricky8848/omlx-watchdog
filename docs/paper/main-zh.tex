...
% 中文支持（需要 XeLaTeX 编译）：使用 TeX Live 自带的 ctex + Fandol 字体
% （系统 PingFang.ttc 为 CFF/OTF，fontspec 无法嵌入 TrueType 字形）
...
\begin{titlepage}
  ...
  \begin{tabular}{rl}
    ...
    日期:     & 2026年9月\\[0.3em]
    许可证:   & MIT \\[0.3em]
    代码仓库: & \url{https://github.com/ricky8848/omlx-watchdog} (v9.1)\\[0.3em]
...
\noindent\textbf{空注册表感知（F4 修复）。} 模型切换后注册表可能暂时为空；$B(t)$ 将 $\varnothing$ 视为健康而非触发重启风暴（v5 时代有 79 次此类事件）。
...
看门狗 + 安装器：\url{https://github.com/ricky8848/omlx-watchdog}（MIT, v9.1; \texttt{curl | bash}）。三层配置：\url{https://github.com/ricky8848/mac-m5-128g-omlx-settings}。完整去标识化跟踪：\texttt{docs/paper/production-data.json}（在工件仓库中）。技术报告（中文）：\url{https://doi.org/10.5281/zenodo.22675074}（Zenodo）。
